...
console.log('===========================================');
console.log('黄氏家族寻根平台 - 阿里云 OSS 部署脚本 (非交互式)');
console.log('===========================================');
...
// 检查必需的环境变量
const bucketName = process.env.ALIYUN_OSS_BUCKET_NAME;
...
if (!bucketName || !accessKeyId || !accessKeySecret) {
  ...
  console.error('  - ALIYUN_OSS_BUCKET_NAME: OSS Bucket 名称');
  console.error('  - ALIYUN_ACCESS_KEY_ID: AccessKeyId');
  console.error('  - ALIYUN_ACCESS_KEY_SECRET: AccessKeySecret');
  console.log('');
  console.log('设置环境变量示例:');
  ...
  console.log('  export ALIYUN_OSS_ENDPOINT="oss-cn-hangzhou.aliyuncs.com"');
  process.exit(1);
}
...
try {
  // 使用环境变量配置 ossutil
  const configDir = path.join(require('os').homedir(), '.ossutilconfig');
