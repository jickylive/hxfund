...
console.log('===========================================');
console.log('黄氏家族寻根平台 - ECS 角色 STS 凭证 OSS 部署脚本');
console.log('===========================================');
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200 && data.startsWith('i-')) {
          ...
          console.log('⚠ 检测到不在 ECS 实例上运行，但仍可尝试获取 STS 凭证');
          resolve(false);
        }
      });
    });
...
function configureOssutil(credentials) {
  console.log('\\n步骤 2: 配置 ossutil 使用 STS 凭证...');
...
// 部署到阿里云 OSS
async function deployToOSS(configFile) {
  console.log('\\n步骤 3: 部署到阿里云 OSS...');
...
async function main() {
  ...
  try {
    ...
    // 配置 ossutil
    const configFile = configureOssutil(stsCredentials);
    
    // 执行部署
    await deployToOSS(configFile);
    ...
    console.error('✗ 获取 STS 凭证或部署失败:', error.message);
    process.exit(1);
  }
}
