...
echo ""
read -p "确认使用以上配置发布到FTP服务器? (y/N): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "发布已取消"
    exit 0
...
elif [ "$FTP_CLIENT" = "ftp" ]; then
    # 使用ftp进行发布（较基础的方式）
    echo "使用 ftp 发布文件..."
    
    # 创建ftp脚本
    TEMP_FTP_SCRIPT=$(mktemp)
    cat > "$TEMP_FTP_SCRIPT" << EOF
...
if [ "$FTP_CLIENT" = "lftp" ]; then
    # 尝试列出远程目录内容
    echo "远程目录内容:"
    lftp -c "open ftp://$FTP_USER:$FTP_PASS@$FTP_HOST:$FTP_PORT; cd $FTP_REMOTE; cls -la"
fi
